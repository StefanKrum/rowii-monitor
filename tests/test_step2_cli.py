...
def test_cross_day_pooled_out_dir_suffixes_non_default_alpha(tmp_path) -> None:
    """Audit fix 2026-08-19: a non-default alpha appends `-a<alpha>`, so alpha
    sweeps never silently overwrite the standard leaf (a refactor had dropped the
    suffix; an alpha-0.10 run then replaced the 0.05 leaf in place). The default
    alpha keeps the plain leaf name, so every existing standard leaf stays
    addressable; the pooled k deliberately never enters the name (no canonical
    constant default, and no historical leaf carried one)."""
    import run_step2
    ...
    assert f(root, "300626-tu", "fusion", alpha=0.05) == base / "fusion-pooled"
    assert f(root, "300626-tu", "fusion", alpha=0.01) == base / "fusion-pooled-a0.01"
    assert f(root, "300626-tu", "fusion", alpha=0.1) == base / "fusion-pooled-a0.1"
    assert (
